...
data_eff <- data %>% filter(!is.na(data))
data_eff
```
...
data_complete <- data_clean %>% filter(species != "whitedams") %>% group_by(species,treatment) %>% summarise(mean_activity = mean(activity), mean_sl = mean(sl), sd_error_activity = std.error(activity),sd_error_sl = std.error(sl))
...
ggplot(data_complete, mapping = aes(x=treatment, y = mean_activity)) +
...
## 用同样的图片，如果不添加进去，另一个人无法利用同样的内容去画图（可能使用其他的图片）。
```
